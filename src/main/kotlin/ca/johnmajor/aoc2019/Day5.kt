...
class IntcodeVM(private val mem: IntArray) {
    ...
    fun run(input: Iterator<Int>): List<Int> {
        var ip: Int? = 0
...
fun main() {
    ...
    val part1 = IntcodeVM(input.toIntArray()).run(listOf(1).iterator()).lastOrNull()
    println("Part 1 Answer: $part1")

    val part2 = IntcodeVM(input.toIntArray()).run(listOf(5).iterator()).lastOrNull()
